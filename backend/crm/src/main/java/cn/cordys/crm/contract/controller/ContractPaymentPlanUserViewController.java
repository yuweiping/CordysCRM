...
@Tag(name = "合同回款计划视图")
@RestController
@RequestMapping("/contract/payment-plan/view")
public class ContractPaymentPlanUserViewController {
    ...
    @Operation(summary = "添加回款计划视图")
    public UserView add(@Validated @RequestBody UserViewAddRequest request) {
        return userViewService.add(request, SessionUtils.getUserId(), OrganizationContext.getOrganizationId(), UserViewResourceType.CONTRACT_PAYMENT_PLAN.name());
    }


    @PostMapping("/update")
    @RequiresPermissions(PermissionConstants.CONTRACT_PAYMENT_PLAN_READ)
    @Operation(summary = "编辑回款计划视图")
    ...
    @RequiresPermissions(PermissionConstants.CONTRACT_PAYMENT_PLAN_READ)
    @Operation(summary = "回款计划视图详情")
    public UserViewResponse viewDetail(@PathVariable String id) {
        return userViewService.getViewDetail(id, SessionUtils.getUserId(), OrganizationContext.getOrganizationId(), FormKey.CONTRACT_PAYMENT_PLAN.getKey());
    }


    @GetMapping("/list")
    @RequiresPermissions(PermissionConstants.CONTRACT_PAYMENT_PLAN_READ)
    @Operation(summary = "回款计划视图列表")
    public List<UserViewListResponse> queryList() {
        return userViewService.list(UserViewResourceType.CONTRACT_PAYMENT_PLAN.name(), SessionUtils.getUserId(), OrganizationContext.getOrganizationId());
    }


    @GetMapping("/fixed/{id}")
    @RequiresPermissions(PermissionConstants.CONTRACT_PAYMENT_PLAN_READ)
    @Operation(summary = "回款计划视图固定/取消固定")
    ...
    @Operation(summary = "回款计划视图-拖拽排序")
    @RequiresPermissions(PermissionConstants.CONTRACT_PAYMENT_PLAN_READ)
    public void editPos(@Validated @RequestBody PosRequest request) {
        userViewService.editPos(request, SessionUtils.getUserId(), UserViewResourceType.CONTRACT_PAYMENT_PLAN.name());
    }


    @GetMapping("/enable/{id}")
    @RequiresPermissions(PermissionConstants.CONTRACT_PAYMENT_PLAN_READ)
    @Operation(summary = "回款计划视图-启用/禁用")
