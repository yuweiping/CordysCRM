...
@Tag(name = "公海视图")
@RestController
@RequestMapping("/pool/account/view")
public class PoolCustomerUserViewController {
    ...
    @Operation(summary = "添加公海视图")
    public UserView add(@Validated @RequestBody UserViewAddRequest request) {
        return userViewService.add(request, SessionUtils.getUserId(), OrganizationContext.getOrganizationId(), UserViewResourceType.CUSTOMER_POOL.name());
    }


    @PostMapping("/update")
    @RequiresPermissions(PermissionConstants.CUSTOMER_MANAGEMENT_POOL_READ)
    @Operation(summary = "编辑公海视图")
